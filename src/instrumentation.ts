 ...
 * Without it nothing is sent anywhere; errors still reach the Netlify function logs.
 */
export async function register() {
  ...
  Sentry.init({
    dsn,
    // Netlify names its deploy context (production, deploy-preview) CONTEXT.
    environment: process.env.CONTEXT ?? process.env.VERCEL_ENV ?? process.env.NODE_ENV,
    release: process.env.COMMIT_REF ?? process.env.VERCEL_GIT_COMMIT_SHA,
